( STC-TorD.muf -FUZZBALL- v1.8G, by Stelard Actek of StelTechCor @ FurryMUCK!
 ...
 the PROGRAMS command. [Note: This isn't required if you set the program M3]
...
arg @ "=" instr if
   arg @ "=" split swap dup not if
      ...
      exit
   then
   match dup pzanfilter if   ( A player=question situation )
...
arg @ "=" instr if
   arg @ "=" split swap dup not if
      ...
      exit
   then
   match dup pzanfilter if   ( A player=dare situation )
      set-daree not if
...
dup dup getlink 2 try
   ...
   exit
endcatch
"~&150<> " swap dup ansify-name "~&150, ~&170" strcat swap dbmostidle 60 / intostr
...
arg @ match dup pzanfilter not if
   ...
   exit
then
dup "J" flag? over getlink "J" flag? and not prog mlevel 3 < and if
   "<> Unfortunatly, due to limitations imposed by the programmer's M# Level," "bold,magenta" textattr .tell
   "<> this player could not be swept home, as either they or their home is not" "bold,magenta" textattr .tell
   "<> set JUMP_OK. Sorry for the inconvenience." "bold,magenta" textattr .tell
   exit
...
: help-header ( -- )
"~&120+----------------------+" ansi-tell
"~&120|  ~&130STC-TorD.muf v1.8G  ~&120|" ansi-tell
...
dup 6 = if
   ...
   " " .tell
   "~&130v1.8G ~&170- ~&110* ~&160Made some changes to take advantage of Delphinus or $lib/dbidle, and" ansi-tell
   "          ~&160the higher programming privelages I have on Spork." ansi-tell
   "          ~&160Though it'll still run on M2." ansi-tell
   " " .tell
   "~&160Contact information on page seven (~&170Q #HELP7~&160)." ansi-tell
then
